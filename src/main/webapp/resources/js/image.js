    ...
    Image.dropZone = function(target_input, custom_zone) {
        ...
        dropZone.live('drop', function (e)
        {
            ...
            if(e.originalEvent.dataTransfer.files[0] != undefined) {
                ...
                if(!isValid) {
                    Main.popDialog('', 'Only PNG, JPG and JPEG files are supported.');
                    return false;
                } else {
            ...
            image.onload = function() {
                ...
                if(w != chk_w || h != chk_h) {
                    if(w >= chk_w && h >= chk_h) {
                        ...
                        Main.popDialog('', 'Please upload an image of size greater than ' + chk_w + 'px X ' + chk_h + 'px.');
                        return false;
                    }
                } else {
                    ...
                    if(img_container == undefined && $('.active_img_container').length == 1) {
                        ...
                        Main.request('/merchant/add_items_images', {itemImages: [src_url]}, callback, {id: $('.active_img_container').eq(0).parent().attr('data-id')});
