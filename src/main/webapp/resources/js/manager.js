    ...
    Manager.listStores = function() {
        ...
        var dataFilter = function (data, type) {
            ...
            for (var i = 0; i < brands.length; i++) {
                ...
                var partnership = brand.merchant.partnershipStatus ? 'Partner' : 'Non-Partner';
                ...
                var row = [brandId, brandName, merchantName, partnership, brand.countStore, brand.featured ? "Featured" : "None", !brand.priority ? "None" : brand.priority, Main.ucfirst(brand.status), actions];
                row = $.extend({}, row);
                tdata.push(row);
            }
        ...
        dataFilter.columns = [
            { "name": "id" },
            { "name": "brandName" },
            { "name": "merchant#businessTitle" },
            { "name": "merchant#partnershipStatus" },
            { "name": "" },
            { "name": "featured" },
            { "name": "priority" },
            { "name": "status" },
...
//                callback.loaderDiv = ".view_courier_boy_map";
    ...
    Manager.fillDboyAccount = function(courierStaff){
        ...
        if(typeof courierStaff.walletAmount != "undefined" && typeof courierStaff.bankAmount != "undefined"){
            ...
            $(".cash_available_balance").text(Main.getFromLocalStorage('currency') + " " + (courierStaff.bankAmount).toFixed(2));
        }
    ...
    Manager.loadTransfers = function() {
        ...
        $('.btn_transfer').live('click', function() {
            ...
            var shopperId = $(this).data('sid');
            var amtTransfer = $(this).data('transfer');
            ...
            var button1 = function() {
                ...
                callback.requestType = "POST";
                var headers = {};
                headers.id = shopperId;

                Main.request('/accountant/update_dboy_account', {advanceAmount: amtTransfer, advanceAmountOrderId: orderId}, callback, headers);
            };
            ...
            var buttons = [button1, button2];
            Main.popDialog('', "Are you sure you want to transfer amount?", buttons);
        ...
        var callback = function (status, data) {
            ...
            if(orders.length > 0) {
                ...
                for(i = 0; i < orders.length; i++) {
                    ...
                    var amtRequested = order.toBeTransferred == undefined ? "" : Main.getFromLocalStorage('currency') + " " + order.toBeTransferred;
                    var amtTransferred = order.transferred == undefined ? "" : Main.getFromLocalStorage('currency') + " " + order.transferred;
                    var amtMerchant = Main.getFromLocalStorage('currency') + " " + order.totalCost;
                    var bankNumber = order.deliveryBoy.bankAccountNumber == undefined ? "" : order.deliveryBoy.bankAccountNumber;
                    var action = '';
                    if(amtRequested != "") {
                        action = '<a href="#" class="btn_transfer" data-transfer="' + order.toBeTransferred + '" data-id="' + orderId + '" data-sid="' + order.deliveryBoy.id + '">Transfer</a>';
                    }
