...
public class MerchantServiceImpl extends AbstractManager implements MerchantService {
    ...
    public PaginationDto getMerchants(RequestJsonDto requestJsonDto) throws Exception {
        ...
        Integer totalRows =  merchantDaoService.getTotalNumberOfMerchants(pageForCount);
    ...
    public void updateStore(RequestJsonDto requestJson) throws Exception {
        ...
        for (StoreEntity store: stores){
            if(store.getId() == null){
                ...
                List<ItemEntity> brandsItems = dbStoresBrand.getItems();
                List<ItemsStoreEntity> newStoresItems = new ArrayList<>();
                for (ItemEntity item: brandsItems){
                    ItemsStoreEntity itemsStore = new ItemsStoreEntity();
                    itemsStore.setItem(item);
                    itemsStore.setStore(store);
                    newStoresItems.add(itemsStore);
                }
                store.setItemsStore(newStoresItems);
                dbStores.add(store);
            } else {
...
//            merchant.getUser().setPassword(GeneralUtil.encryptPassword(headerDto.getPassword()));
   ...
   private Boolean partnershipStatus;
        * */
        ...
        assoc.put("merchant", "id,businessTitle,partnershipStatus");
    ...
    public PaginationDto findBrands(HeaderDto headerDto, RequestJsonDto requestJsonDto) throws Exception {
        ...
        if (headerDto.getMerchantId() != null){
             totalRows =  merchantDaoService.getTotalNumberOfBrandByMerchant(headerDto.getMerchantId(), pageForCount);
        }else{
             totalRows =  merchantDaoService.getTotalNumberOfBrand(pageForCount);
        }
        paginationDto.setNumberOfRows(totalRows);
...
*/
    ...
    public List<CategoryEntity> findParentCategoriesItems(RequestJsonDto requestJson) throws Exception {
        ...
        String fields = "id,name,priority,item";
    ...
    public PaginationDto getOrders(HeaderDto headerDto, RequestJsonDto requestJson) throws Exception {
        ...
        String fields = "id,orderName,orderStatus,deliveryStatus,orderDate,customer,orderVerificationCode,store,deliveryBoy,deliveryBoySelections,assignedTime,systemServiceCharge,deliveryCharge,paidFromCOD,paidFromWallet,attachments,itemServiceAndVatCharge,grandTotal,totalCost,discountFromStore,rating,paymentMode,deliveryCharge,forceAssigned,bill,itemsOrder";
    ...
    public PaginationDto getPurchaseHistory(HeaderDto headerDto, RequestJsonDto requestJson) throws Exception {
        ...
        String fields = "id,orderName,deliveryStatus,orderStatus,attachments,orderDate,itemServiceAndVatCharge,customer,store,deliveryBoy,totalCost,discountFromStore";
